...
def main(reference_ephys_file, prediction_ephys_file, reference_label_file,
         label_key, morph_file, output_file, n_trees, **kwargs):
    ...
    # drop values that don't have labels
    nan_mask = ~pd.Series(labels).isnull().values
    labels = labels[nan_mask]
    ref_df = ref_df.loc[nan_mask, :]

    pred_labels = pred.rf_predict(ref_df, labels, ephys_pred, n_trees=n_trees)


    logging.debug("Saving results")
    pd.DataFrame(pred_labels, index=ephys_pred.index.values).to_csv(output_file)
